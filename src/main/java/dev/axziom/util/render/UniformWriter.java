...
import dev.axziom.mixin.render.GLBufferAccessor;
import dev.axziom.mixin.render.PostChainAccessor;
import dev.axziom.mixin.render.PostPassAccessor;
...
public class UniformWriter
{
    ...
    public void setUniforms(PostChain chain, Map<String, List<UniformValue>> configs)
    {
        ...
        List<PostPass> passes = ((PostChainAccessor) chain).jewdust$getPasses();
        for (PostPass pass : passes)
        {
            Map<String, GpuBuffer> uniformBuffers = ((PostPassAccessor) pass).jewdust$getUniformBuffers();
            for (Map.Entry<String, List<UniformValue>> entry : configs.entrySet())
            {
                ...
                int handle    = ((GLBufferAccessor) dest).jewdust$getHandle();
