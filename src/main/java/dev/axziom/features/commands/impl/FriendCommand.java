...
import dev.axziom.manager.FriendManager;
...
import static dev.axziom.features.commands.argument.FriendArgumentType.friend;
import static dev.axziom.features.commands.argument.FriendArgumentType.getFriend;
...
public class FriendCommand extends Command {
    ...
    public void createArgumentBuilder(LiteralArgumentBuilder<CommandManager> builder) {
        builder.then(literal("list")
                        .executes((ctx) -> {
                            List<String> friends = JewDust.friendManager.getFriends();
                            if (friends.isEmpty()) {
                                return success("You have no friends :(");
                            }
                            StringJoiner joiner = new StringJoiner(", ");
                            for (String name : friends) {
                                long expiry = JewDust.friendManager.getExpiry(name);
                ...
                .then(literal("clear")
                        .executes((ctx) -> {
                            JewDust.friendManager.clearFriends();
                ...
                .then(literal("remove")
                        .then(argument("username", friend())
                                .executes((ctx) -> {
                                    ...
                                    if (!JewDust.friendManager.isFriend(username)) {
                                        return success("{green} %s {reset} is not on your friends list.", username);
                                    }
                                    JewDust.friendManager.removeFriend(username);
    ...
    private int addPlayer(String username, long expiry) {
        ...
        JewDust.friendManager.addFriend(username, expiry);
        if (expiry == FriendManager.PERMANENT) {
    ...
    private int addRadius(int blocks, long expiry) {
        ...
        for (Player player : mc.level.players()) {
            ...
            if (JewDust.friendManager.isFriend(name)) continue;
            JewDust.friendManager.addFriend(name, expiry);
            added++;
        }
        if (added == 0) {
