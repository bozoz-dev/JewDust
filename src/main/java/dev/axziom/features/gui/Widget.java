...
import dev.axziom.util.render.ScissorUtil;
...
public class Widget extends Feature {
    ...
    public void drawScreen(GuiGraphics context, int mouseX, int mouseY, float partialTicks) {
        ...
        int screenHeight = (int) (mc.getWindow().getGuiScaledHeight() / JewDustGui.getScale());
        ...
        if (animatedHeight > 0.5f) {
            ...
            for (Item item : this.getItems()) {
                ...
                if (GuiNavigator.isSelected(item) && !(item instanceof dev.axziom.features.gui.items.buttons.ModuleButton)) {
            ...
            for (Item item : this.getItems()) {
                if (item instanceof dev.axziom.features.gui.items.buttons.ModuleButton mb && mb.isAnimating()) {
                    anyAnimating = true;
                    break;
                }
            }
    ...
    private void scrollSelectionIntoView(float visibleHeight, float maxScroll) {
        ...
        for (Item item : this.getItems()) {
            ...
            if (item instanceof dev.axziom.features.gui.items.buttons.ModuleButton mb && mb.isSubOpen()) {
                float settingTop = contentTop + GuiTheme.MODULE_HEIGHT
                        + (mb.hasMultiplePages() ? dev.axziom.features.gui.items.buttons.ModuleButton.SWITCHER_HEIGHT : 0);
