...
public class AutoMineModule extends Module {
    ...
    private final Setting<Color> glassFillColor = color("GlassFillColor", 145, 79, 220, 255).setPage("Render")
            .setVisibility(v -> glassRender.getValue());
    private final Setting<Color> glassOutlineColor = color("GlassOutlineColor", 145, 79, 220, 255).setPage("Render")
            .setVisibility(v -> glassRender.getValue());
    ...
    private void onMineFinish(BlockPos pos) {
        ...
        AutoCrystalModule crystal = JewDust.moduleManager.getModuleByClass(AutoCrystalModule.class);
    ...
    private boolean placeGlass(BlockPos pos) {
        ...
        if (!JewDust.placementManager.enqueue(pos, Items.GLASS)) return false;
    ...
    private void onRenderDebug(Render3DEvent event) {
        ...
        for (CheckPos cp : checkPos) {
            ...
            RenderUtil.drawBox(event.getMatrix(), cp.blockPos, new Color(145, 79, 220, alpha), 1.0f);
        }
    }

    private SpeedMineModule speedMine() {
        SpeedMineModule m = JewDust.moduleManager.getModuleByClass(SpeedMineModule.class);
