...
public class AutoSwordModule extends Module {
    ...
    private void onPreTick(PreTickEvent event) {
        ...
        switch (tpsMode.getValue()) {
            case LATEST -> tps = JewDust.tpsCounterService.getLatestTPS();
            case AVERAGE -> tps = JewDust.tpsCounterService.getAverageTPS();
            default -> tps = 20f;
        }
        ...
        if (!isMaceAttackReady()) {
            AutoCrystalModule ac = JewDust.moduleManager.getModuleByClass(AutoCrystalModule.class);
        ...
        boolean needSwap = weaponSlot != JewDust.swapManager.serverSlot();
        ...
        boolean swung = JewDust.swapManager.withSwap(weapon, SwapMode.SILENT, SwapPriority.WEAPON, () -> {
            if (doCrit) {
    ...
    public void onRender3D(Render3DEvent event) {
        ...
        RenderUtil.drawBox(event.getMatrix(), box, JewDust.colorManager.get("ui"), 1.5f);
    }
