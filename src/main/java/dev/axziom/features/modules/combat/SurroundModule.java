...
public class SurroundModule extends Module {
    ...
    private final PlacementManager.PlacementListener airRefillListener = (pos, nowAir) -> {
        ...
        if (JewDust.placementManager.enqueue(pos, Items.OBSIDIAN)) {
            ownedQueued.add(pos);
        }
    ...
    public void onEnable() {
        JewDust.placementManager.addListener(airRefillListener);
    }

    @Override
    public void onDisable() {
        JewDust.placementManager.removeListener(airRefillListener);
        JewDust.placementManager.removeQueuedFor(ownedQueued::contains);
        ownedQueued.clear();
        wantedPoses.clear();
        fireworkPoses.clear();
        extendPoses.clear();
    ...
    private void onTick(TickEvent event) {
        ...
        if (!ownedQueued.isEmpty()) {
            JewDust.placementManager.removeQueuedFor(p -> ownedQueued.contains(p) && !wantedPoses.contains(p));
        ...
        if (JewDust.placementManager.useFireworks(fireworkUsePoses, Direction.DOWN)) {
            ...
            if (crystal != null) {
                ...
                for (BlockPos p : wantedPoses) {
                    if (Math.abs(p.getX() - crystalCell.getX()) <= 1
                        ...
                        JewDust.placementManager.forceResetPlaceCooldown(p);
                    }
                }
            }
        }
    ...
    private void applyMineProtect(List<BlockPos> feetPositions, List<BlockPos> placePoses) {
        BreakIndicatorsModule indicators = JewDust.moduleManager.getModuleByClass(BreakIndicatorsModule.class);
        ...
        for (BreakIndicatorsModule.BreakInfo info : indicators.getActiveBreaksSnapshot().values()) {
            ...
            if (player == null || player == mc.player || JewDust.friendManager.isFriend(player)) continue;
    ...
    private void buildOpponentSurroundPoses() {
        ...
        for (Player p : mc.level.players()) {
            ...
            if (JewDust.friendManager.isFriend(p)) continue;
