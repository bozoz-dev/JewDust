...
public class ElytraAssistModule extends Module {
    ...
    private void resubmit() {
        JewDust.rotationManager.submit(new RotationRequest(ID, PRIORITY, lastYaw, lastPitch, RotationRequest.Mode.MOTION, true, true));
    }
    ...
    private void stop() {
        clearTicks = 0;
        if (steering) {
            JewDust.rotationManager.cancel(ID);
            steering = false;
        }
    }
