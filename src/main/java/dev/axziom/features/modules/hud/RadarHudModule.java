...
public class RadarHudModule extends HudModule {
    ...
    private static final Identifier LOGO = Identifier.fromNamespaceAndPath("jewdust", "textures/font/logo_white.png");
    ...
    public void render(Render2DEvent event) {
        ...
        for (int i = entries.size() - 1, row = 0; i >= 0; i--, row++) {
            ...
            int pops = JewDust.playerInfoManager.getTotemPops(entry.player.getUUID());
    ...
    private int resolveNameColor(HudClientModule hudClient, Player player) {
        ...
        if (JewDust.friendManager.isFriend(player)) return hudClient.radarFriendColor.getValue().getRGB();
