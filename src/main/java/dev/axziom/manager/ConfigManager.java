...
import dev.axziom.features.settings.EnumConverter;
...
public class ConfigManager {
    ...
    private static final Path JEWDUST_PATH = FabricLoader.getInstance().getGameDir().resolve("jewdust");
    ...
    public void load() {
        ...
        for (Jsonable jsonable : jsonables) {
            try {
                String read = Files.readString(JEWDUST_PATH.resolve(jsonable.getFileName()));
    ...
    public void save() {
        ...
        for (Jsonable jsonable : jsonables) {
            try {
                ...
                Files.writeString(JEWDUST_PATH.resolve(jsonable.getFileName()), GSON.toJson(json));
            } catch (Throwable e) {
    ...
    private void mkdirs() {
        if (!JEWDUST_PATH.toFile().exists()) {
            boolean success = JEWDUST_PATH.toFile().mkdirs();
            if (!success) {
